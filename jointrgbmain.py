...
# Define a function to calculate the RGB difference
# Define a function to calculate the RGB difference
'''
def diff(img1, img2):
          ...
          for i, bbox in enumerate(bboxes1):
            ...
            if i < len(bbox_labels):
               ...
               if camera2matchup is not None:
                  ...
                  cv.putText(frame2_annotated, custom_text, (text_X, text_Y), cv.FONT_HERSHEY_SIMPLEX, 0.5, bbox_color[i], 2, cv.LINE_AA)

            text_x = x1
            text_y = y1 - 10 if y1 - 10 > 10 else y1 + 10
            cv.putText(frame1_annotated, custom_text, (text_x, text_y), cv.FONT_HERSHEY_SIMPLEX, 0.5, bbox_color[i], 2, cv.LINE_AA)
