         ...
         for i, joint2 in enumerate(joints2):
          if joint2.all != None and np.any(joint2 != [0,0]):
            ...
            for pickcolor in bbox_color:
              if pickcolor not in people2.values():
                color = pickcolor
            new, val =  diff2(joint2,prevbboxes2)
            if val :
         
             update_key_by_rgb(people2,bbox,color)
             ...
             if new != (0,0,0,0):
              #print("PEOPLE2")
              #print(people2)
              inputCOORD = find_closest_element2(people1,findclosest)
              rgbofOG = people2[inputCOORD]
              #print("I am rgb of closest")
              #print(rgbofOG)
              update_key_by_rgb(people2,bbox,rgbofOG)
              color = rgbofOG
            cv.rectangle(frame2_annotated, (x1, y1), (x2, y2), color, 2)  # Draw the bounding box with custom color
            #print("PEOPLE2")
            #print(people2)
        ...
        if cameraval == 2: 
         for i, joint1 in enumerate(joints1):
          if joint1.all != None and np.any(joint1 != [0,0]):
            ...
            color =[]
            for pickcolor in bbox_color:
              if pickcolor not in people1.values():
                color = pickcolor
            new, val =  diff2(joint1,prevbboxes1)
            if val :
         
             update_key_by_rgb(people1,bbox,color)
             ...
             if new != (0,0,0,0):
              ...
              #print(people1)
              inputCOORD = find_closest_element2(people1,findclosest)
              rgbofOG = people1[inputCOORD]
              #print("I am rgb of closest")
              #print(rgbofOG)
              update_key_by_rgb(people1,bbox,rgbofOG)
              color = rgbofOG
            cv.rectangle(frame1_annotated, (x1, y1), (x2, y2), color, 2)  # Draw the bounding box with custom color
            #print("PEOPLE1")
            #print(people1)
